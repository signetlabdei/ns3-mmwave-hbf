...
MmwaveThreeGppAntennaInitializationTestCase::DoRun (void)
{
  ...
  double freq[] {6.0, 28.0}; // frequency of the CCs
  ...
  for (auto ccIndex : mmWaveEnbDev->GetCcMap ())
  {
    ...
    Ptr<AntennaModel> antenna = mmWaveSpectrumPhy->GetRxAntenna ();

    // check if the helper created the correct antenna type
    Ptr<AntennaArrayModel> aAm = DynamicCast<AntennaArrayModel> (antenna);
    NS_LOG_DEBUG ("Antenna object " << aAm << " for CC " << (uint16_t)ccIndex.first);

    if (!aAm)
    {
      NS_TEST_ASSERT_MSG_EQ (false, true, "The antenna model was not properly configured");
    }

    // check if the number of antenna elements has been configured
    uint8_t dim1 = aAm->GetAntennaNumDim1 ();
    NS_TEST_ASSERT_MSG_EQ (dim1, sqrt (mmWaveEnbDev->GetAntennaNum ()), "The number of antenna elements was not properly configured");
    uint8_t dim2 = aAm->GetAntennaNumDim2 ();
    NS_TEST_ASSERT_MSG_EQ (dim2, sqrt (mmWaveEnbDev->GetAntennaNum ()),"The number of antenna elements was not properly configured");
  ...
  for (auto ccIndex : mmWaveUeDev->GetCcMap ())
  {
    ...
    // check if the number of antenna elements has been configured
    uint8_t dim1 = aAm->GetAntennaNumDim1 ();
    NS_TEST_ASSERT_MSG_EQ (dim1, sqrt (mmWaveUeDev->GetAntennaNum ()), "The number of antenna elements was not properly configured");
    uint8_t dim2 = aAm->GetAntennaNumDim2 ();
    NS_TEST_ASSERT_MSG_EQ (dim2, sqrt (mmWaveUeDev->GetAntennaNum ()),"The number of antenna elements was not properly configured");
  }
}
