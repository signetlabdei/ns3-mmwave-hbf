...
MmwaveThreeGppChannelInitializationTestCase::DoRun (void)
{
  ...
  for (auto ccIndex : mmWaveEnbDev->GetCcMap ())
  {
    ...
    threeGppSplm->GetAttribute ("Frequency", splmFreq);
    ...
    NS_TEST_ASSERT_MSG_EQ (ccm, threeGppSplm->GetChannelConditionModel (), "The channel condition model associated to the propagation loss model is not the same as the one associated to the spectrum propagation loss model");
  }
}
