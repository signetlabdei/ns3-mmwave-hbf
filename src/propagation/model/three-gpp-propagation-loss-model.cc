...
ThreeGppPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ThreeGppPropagationLossModel")
    ...
    .AddAttribute ("Frequency", "The centre frequency in Hz.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&ThreeGppPropagationLossModel::m_frequency),
    ...
    .AddAttribute ("ShadowingEnabled", "Enable/disable shadowing.",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("UpdatePeriod", "Update the shadow fading after this time.",
                                             ...
                                             Ptr<MobilityModel> b) const
{
  ...
  NS_ASSERT_MSG (m_frequency >= 500.0e6 && m_frequency <= 100.0e9, "Frequency should be between 0.5 and 100 GHz");
...
ThreeGppPropagationLossModel::GetShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  ...
  // look for the shadowing value in m_shadowingMap
  bool update = false; // indicates if the shadowing value has to be updated
  ...
  if (m_shadowingMap.find (key) != m_shadowingMap.end ())
    {
      ...
      shadowingValue = m_shadowingMap.at (key).m_shadowing;

      // check if it has to be updated:
      // 1. check if the update period exceeded (update period = 0 means never update)
      // 2. check if the channel condition has changed
      if ((m_updatePeriod.GetNanoSeconds () != 0.0 && Simulator::Now () - m_shadowingMap.at (key).m_generatedTime > m_updatePeriod) ||
          m_shadowingMap.at (key).m_condition != cond)
        {
          update = true;
        }
    }
    ...
    {
      // shadowing value not found for this channel
      notFound = true;
    }

  if (notFound || update)
    {
      // compute the shadowing value for this channel
      shadowingValue = ComputeShadowing (a, b, cond);

      // store the shadowing value in the map
      ShadowingMapItem item;
      item.m_shadowing = shadowingValue;
      item.m_generatedTime = Simulator::Now ();
      item.m_condition = cond;
      m_shadowingMap [key] = item;
    }
...
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_frequency <= 7.0e9, "RMa scenario is valid for frequencies between 0.5 and 7 GHz");
...
double
ThreeGppRmaPropagationLossModel::ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  ...
  if (cond == ChannelCondition::LosConditionValue::LOS)
    {
      ...
      if (distance2d <= distanceBp)
        {
          shadowingLoss = m_normRandomVariable->GetValue () * 4.0;
        }
      else
        {
          shadowingLoss = m_normRandomVariable->GetValue () * 6.0;
        }
    }
  else if (cond == ChannelCondition::LosConditionValue::NLOS)
    {
      shadowingLoss = m_normRandomVariable->GetValue () * 8.0;
    }
  else
    {
      NS_FATAL_ERROR ("Unknown channel condition");
...
double
ThreeGppUmaPropagationLossModel::ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  double shadowingLoss;
...
double
ThreeGppUmiStreetCanyonPropagationLossModel::ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  double shadowingLoss;
  ...
  if (cond == ChannelCondition::LosConditionValue::LOS)
    ...
    {
      shadowingLoss = m_normRandomVariable->GetValue () * 7.82;
    }
  else
    {
      NS_FATAL_ERROR ("Unknown channel condition");
...
double
ThreeGppIndoorOfficePropagationLossModel::ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const
{
  double shadowingLoss;

  if (cond == ChannelCondition::LosConditionValue::LOS)
    {
      shadowingLoss = m_normRandomVariable->GetValue () * 3.0;
    }
  else if (cond == ChannelCondition::LosConditionValue::NLOS)
    {
      shadowingLoss = m_normRandomVariable->GetValue () * 8.03;
    }
  else
    {
      NS_FATAL_ERROR ("Unknown channel condition");
