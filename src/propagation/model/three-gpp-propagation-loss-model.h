...
private:
   ...
   * \param za the height of the first node
   * \param zb the height of the second node
   ...
   * If not found or if it has to be updated it generates a new independent
   * realization and stores it in the map.
   * \param a tx mobility model
   * \param b rx mobility model
  ...
  /**
   * \brief Generates a shadowing realization.
   * \param a tx mobility model
   * \param b rx mobility model
   * \param cond the LOS/NLOS channel condition
   * \return shadowing loss in dB
   */
  virtual double ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const = 0;
...
protected:
  ...
  */
  ...
  struct ShadowingMapItem
  {
    ...
    Time m_generatedTime; //!< the generation time
    ChannelCondition::LosConditionValue m_condition; //!< the LOS/NLOS condition
  };

  mutable std::map<uint32_t, ShadowingMapItem> m_shadowingMap; //!< map to store the shadowing values
  Time m_updatePeriod; //!< defines the update period for the shadow fading
};
...
private:
   ...
   * \return shadowing loss in dB
   */
  virtual double ComputeShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b, ChannelCondition::LosConditionValue cond) const override;
   ...
   * \param h the average street width in meters
