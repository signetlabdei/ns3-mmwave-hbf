...
ThreeGppChannelConditionModelTestCase::ThreeGppChannelConditionModelTestCase ()
  : TestCase ("Test case for the ThreeGppRmaChannelConditionModel"), m_testVectors ()
{
}
  ...
  for (uint32_t i = 0; i < m_testVectors.GetN (); ++i)
    {
      ...
      NS_LOG_DEBUG ("numLos " << m_numLos << " numberOfReps " << numberOfReps << " resultPlos " << resultPlos << " ref " << testVector.m_pLos);
      NS_TEST_EXPECT_MSG_EQ_TOL (resultPlos, testVector.m_pLos, testVector.m_tolerance, "Got unexpected LOS probability");
    }
}
...
ChannelConditionModelsTestSuite::ChannelConditionModelsTestSuite ()
  : TestSuite ("channel-condition-model", UNIT)
{
